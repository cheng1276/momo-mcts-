...
/* ============================================================
   ABC 冒險島 — 蒙蒙老師的 MCTS 英文課（6 歲・寓教於樂）
...
function stageMode(w, word) {
  if (w.seen === 0) return "teach";      // 新內容：先教，不考
...
// ---------- 蒙蒙老師的 MCTS 備課 ----------
// 候選：快忘記的複習＋新字＋補信心；動作＝選哪個字（題型由階段決定）
function candidatesOf(st, poolIds) {
  const now = st.q;
  const seen = [], unseen = [];
  for (const id of poolIds) (st.ws[id].seen > 0 ? seen : unseen).push(id);
  ...
  const newN = st.E > 70 ? 4 : st.E > 55 ? 3 : st.E > 35 ? 2 : 1;
  const uw = [], ud = [];
  ...
  while (wi2 < uw.length || di < ud.length) {           // 單字:對話 ≈ 2:1 穿插
    for (let k = 0; k < 2 && wi2 < uw.length; k++) unseenSorted.push(uw[wi2++]);
    if (di < ud.length) unseenSorted.push(ud[di++]);
  }
  for (const id of unseenSorted.slice(0, newN)) out.push({ id, tag: "new" });
  if (st.wrongStreak >= 1 && seen.length) {
    let best = seen[0], bp = -1;
    for (const id of seen) {
      const word = ALL[id];
      const p = predictP(st.ws[id], word, now, st.E, stageMode(st.ws[id], word));
      if (p > bp) { bp = p; best = id; }
    }
    out.push({ id: best, tag: "confidence" });
  }
  const hasDlgPool = poolIds.some(id => ALL[id].kind === "dlg");
  if (hasDlgPool && !out.some(c => ALL[c.id].kind === "dlg")) {
    if (ud.length) out.push({ id: ud[0], tag: "new" });
    else {
  ...
  return dedup.length ? dedup : poolIds.slice(0, 1).map(id => ({ id, tag: "review" }));
}
function rolloutTeach(st, poolIds, total) {
  let steps = 0;
  while (st.q < total && steps++ < 4) {
    const cands = candidatesOf(st, poolIds);
    const r = rnd();
    let c;
    if (r < 0.6) c = cands[0];
    else c = pick(cands);
...
function teacherDecide(st0, poolIds, total, iters) {
  ...
  const rootCands = candidatesOf(st0, poolIds);
  ...
  for (let it = 0; it < iters; it++) {
    ...
    const v = rolloutTeach(st, poolIds, total);
...
// ---------- 儲存（跨次進度：貼紙簿才有意義）----------
const SAVE_KEY = "abc-adventure-v2";
...
const TAGS = { review: { t: "複習快忘記的", c: K.teal }, new: { t: "教一個新單字", c: K.sun }, confidence: { t: "補一題信心", c: K.pink } };
...
export default function App() {
  ...
  async function saveAll(st, tStars, stk, ses) {
    const it = {};
    ALL.forEach((item, i) => { if (st.ws[i].s > 0.02) it[`${item.kind}:${item.en}`] = st.ws[i].s; });
    await persist({ it, stars: tStars, stickers: stk, sessions: ses, t: Date.now() });
  }

  const mastered = ALL.filter(w => learner.ws[w.id].s >= 0.7);

  function startSession(islId) {
    const isl = ISLANDS.find(i => i.id === islId);
    const ids = ALL.filter(w => !isl.cats || isl.cats.includes(w.cat)).map(w => w.id);
    const st = cloneL(learnerRef.current);
    st.q = 0; st.wrongStreak = 0; st.tc = 0; st.E = clamp(st.E * 0.3 + 55, 40, 90);
    st.ws.forEach(w => { w.last = Math.min(w.last, -3); });
  ...
  function goNext() {
    fx("tap");
    const num = qNum + 1;
    if (num >= Q_PER_SESSION) {
      fx("fanfare");
      const ses = sessions + 1;
      setSessions(ses);
      saveAll(learnerRef.current, totalStars, stickers, ses);
      setPhase("done");
    } else {
      setPhase("play");
          ...
          <div>
            <header style={{ textAlign: "center", margin: "8px 0 14px" }}>
              ...
              <p style={{ margin: "4px 0 0", fontSize: 13, color: "#f2fbff", fontWeight: 700 }}>
                蒙蒙老師陪你學單字、聊英文——集星星、換貼紙！
              </p>
            </header>
            <Card>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
                <div style={{ fontWeight: 900, fontSize: 14 }}>⭐ 星星 <span style={{ fontFamily: MONO }}>{totalStars}</span>　🏅 學會 <span style={{ fontFamily: MONO }}>{mastered.length}</span> 個字</div>
                <Btn onClick={() => { fx("tap"); setPhase("book"); }} color="#fff" edge={K.edge} style={{ padding: "8px 12px", fontSize: 13 }}>📒 貼紙簿</Btn>
              </div>
            </Card>
            <Card>
              <div style={{ fontWeight: 900, fontSize: 15, marginBottom: 10, textAlign: "center" }}>今天要去哪個島冒險？</div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                {ISLANDS.map(isl => (
                  <Btn key={isl.id} onClick={() => { fx("tap"); startSession(isl.id); }}
                    color={isl.id === "all" ? K.sun : "#fff"} edge={isl.id === "all" ? K.sunEdge : K.edge}
                    style={{ fontSize: 15, padding: "14px 8px", gridColumn: isl.id === "all" ? "1 / -1" : "auto" }}>
                    <span style={{ fontSize: 22 }}>{isl.icon}</span> {isl.name}
                    {isl.id === "all" && <div style={{ fontSize: 11, color: K.sub, fontWeight: 700 }}>（讓蒙蒙老師決定）</div>}
                  </Btn>
                ))}
              </div>
              {!hasTTS() && <div style={{ fontSize: 11.5, color: K.sub, marginTop: 8 }}>※ 這台裝置沒有語音功能，會改用文字出題</div>}
            </Card>
            <ParentNotes learner={learner} notes={null} sessions={sessions} onReset={async () => {
              const fresh = freshLearner();
              setLearner(fresh); learnerRef.current = fresh;
              setTotalStars(0); setStickers([]); setSessions(0);
              await persist({ it: {}, stars: 0, stickers: [], sessions: 0, t: Date.now() });
            }} />
          </div>
        )}
          ...
          <div>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
              <div style={{ fontSize: 15 }}>
                {Array.from({ length: Q_PER_SESSION }, (_, i) => (
            ...
            {phase === "reveal" && (
              <div style={{ textAlign: "center", marginTop: 12 }}>
                ...
                <Btn onClick={goNext} color={K.teal} edge={K.tealEdge} fg="#fff" style={{ fontSize: 17, padding: "13px 34px" }}>
                  {qNum + 1 >= Q_PER_SESSION ? "🎁 看看今天的成果！" : "下一題 ▶"}
                </Btn>
              </div>
            )}

            <div style={{ marginTop: 12 }}>
              <ParentNotes learner={learner} notes={notes} sessions={sessions} compact />
            </div>
          </div>
        )}
        ...
        {phase === "done" && (
          <div>
            <Card style={{ textAlign: "center", animation: "popIn .5s ease" }}>
              ...
              <div style={{ fontSize: 22, fontWeight: 900, margin: "4px 0" }}>今天的冒險完成！</div>
              ...
              <div style={{ display: "flex", gap: 10, justifyContent: "center", flexWrap: "wrap" }}>
                <Btn onClick={() => { fx("tap"); setPhase("home"); }} color="#fff" edge={K.edge}>🏝 回小島</Btn>
                <Btn onClick={() => { fx("tap"); setPhase("book"); }} color={K.pink} edge="#c2588a" fg="#fff">📒 貼紙簿</Btn>
                <Btn onClick={() => { fx("tap"); startSession(island); }} color={K.teal} edge={K.tealEdge} fg="#fff">🔁 再冒險一次</Btn>
              </div>
            </Card>
            <ParentNotes learner={learner} notes={notes} sessions={sessions} />
          </div>
        )}
          ...
          <div>
            <Card>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
                ...
                <Btn onClick={() => { fx("tap"); setPhase("home"); }} color="#fff" edge={K.edge} style={{ padding: "7px 12px", fontSize: 13 }}>↩ 回小島</Btn>
              </div>
...
// ---------- 家長／老師透視面板 ----------
function ParentNotes({ learner, notes, sessions, compact, onReset }) {
  const now = learner.q;
        ...
        <div style={{ fontSize: 12.5, lineHeight: 1.8, marginTop: 8, color: K.ink }}>
          {notes && notes.chosen && (
            <div style={{ background: K.cream, borderRadius: 12, padding: "8px 10px", marginBottom: 8 }}>
              <b>這一題為什麼出「{ALL[notes.chosen.id].en}」？</b>
              ...
              {notes.forced && <span style={{ background: K.purple, color: "#fff", borderRadius: 8, padding: "1px 8px", marginLeft: 4, fontSize: 11, fontWeight: 900 }}>💬 本堂聊天保底</span>}
              <div style={{ fontSize: 11.5, color: K.sub, marginTop: 4 }}>
                老師把「剩下的整堂課」模擬了 {notes.total} 遍（{notes.ms?.toFixed(0)} ms），比較每個候選字帶來的
                「明日記憶保留＋興趣」期望值後拍板。目前興趣電量估計 {notes.E?.toFixed(0)}%。
              </div>
          ...
          {!compact && (
            <>
              ...
              <div style={{ fontSize: 11.5, color: K.sub }}>
                <b>老實說明</b>：「模擬器」是遺忘曲線＋測驗效應＋間隔效應＋興趣電量的<b>近似模型</b>——跟本系列醫療 App 的教訓一樣，模型品質是天花板；但這裡每答一題就校正一次、出錯代價低。我們用 80 位「參數刻意不同的合成孩子」考過這位老師：對比隨機出題，MCTS 讓<b>真正學會的內容多 15~30%、期末興趣高 10 點以上、挫折收場趨近 0</b>，代價是「淺淺沾過」的量較少——它選擇深耕。單字與對話由同一套記憶引擎排課（對話約佔 12%）。口說部分採「聽＋跟唸」，語音辨識不在本版範圍。三個設計也是模擬逼出來的：新字<b>先教不考</b>（否則挫折爆表）、介紹卡<b>連塞會膩</b>（新鮮感遞減）、複習挑<b>答對率約七成</b>的甜蜜點。連錯時塞一題會的補信心，不是 bug，是模擬結果。已上課 {sessions} 次；進度自動儲存。
              </div>
              {onReset && (
