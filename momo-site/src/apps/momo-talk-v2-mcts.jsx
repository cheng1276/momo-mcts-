...
async function askMomo(history, childText, learned) {
  ...
  // 在 Claude 預覽視窗裡可直接呼叫；放到自己的網站要走中繼站（index.html 的 window.MOMO_CHAT_PROXY）
  const proxy = (typeof window !== "undefined" && window.MOMO_CHAT_PROXY) || "";
...
export default function App() {
  ...
  async function chatSend(text) {
    ...
    try { rep = await askMomo(chatHist.current, t, learnedLines()); }
      ...
      const noProxy = typeof window !== "undefined" && !window.MOMO_CHAT_PROXY && !/claude/i.test(window.location.hostname + document.referrer);
      rep = { en: "Sorry, can you say that again?", zh: noProxy ? "（自己的網站上，聊天需要先設定中繼站——見 worker/README）" : "抱歉，可以再說一次嗎？（連線不順）", hints: defaultHints(), raw: "" };
    }
