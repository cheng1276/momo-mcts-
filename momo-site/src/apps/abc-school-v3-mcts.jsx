...
/* ============================================================
   ABC 蒙蒙學校 3.0 會話擴充版 — 多輪對話＋開口說（6 歲）
   ・會話升級：18 個生活場景（打招呼/禮貌/學校/吃喝/玩耍/家/心情），
     一景 1~2 輪你來我往；第三階段「開口說」只給中文情境，
     孩子要憑記憶選出該說的英文（從聽懂 → 會說）
   ・2.0 修正：語音「說完才換」——TTS Promise 化＋完成偵測
     （onend＋估時保險絲），換卡/下一題都會等蒙蒙好好講完
...
const RAW = [
  ...
  ["fox", "狐狸", "🦊", "animal", 1], ["zebra", "斑馬", "🦓", "animal", 2],
  // 點心島
  ...
  ["tea", "茶", "🍵", "food", 1], ["ice cream", "冰淇淋", "🍨", "food", 3],
  // 彩虹數字島
  ...
  ["orange", "橘色", "🟠", "colorNum", 2], ["brown", "咖啡色", "🟤", "colorNum", 2],
  ...
  // 衣物玩具島
  ["hat", "帽子", "🧢", "wear", 1], ["shoes", "鞋子", "👟", "wear", 2], ["socks", "襪子", "🧦", "wear", 2],
  ["shirt", "上衣", "👕", "wear", 2], ["pants", "褲子", "👖", "wear", 2], ["shorts", "短褲", "🩳", "wear", 2],
  ["dress", "洋裝", "👗", "wear", 2], ["coat", "外套", "🧥", "wear", 2], ["vest", "背心", "🦺", "wear", 1],
  ["queen", "皇后", "👸", "wear", 2], ["teddy bear", "泰迪熊", "🧸", "wear", 2],
  ["kite", "風箏", "🪁", "wear", 1], ["robot", "機器人", "🤖", "wear", 2], ["balloon", "氣球", "🎈", "wear", 2],
  // 出門島
  ["bike", "腳踏車", "🚲", "out", 1], ["train", "火車", "🚆", "out", 2], ["plane", "飛機", "✈️", "out", 2],
  ["boat", "船", "🚤", "out", 1], ["park", "公園", "🏞️", "out", 1], ["mountain", "山", "⛰️", "out", 3],
  // 心情島
  ...
  ["door", "門", "🚪", "life", 1], ["window", "窗戶", "🪟", "life", 2], ["bed", "床", "🛏️", "life", 1],
  ["phone", "電話", "📱", "life", 1], ["cup", "杯子", "☕", "life", 1], ["umbrella", "雨傘", "☂️", "life", 3],
  ["box", "盒子", "📦", "life", 1], ["wind", "風", "🌬️", "life", 1], ["x-ray", "X光", "🩻", "life", 2],
  // 動一動島
  ...
  ["fast", "快", "🏎️", "act", 1], ["slow", "慢", "🐌", "act", 1],
];
const WORDS = RAW.map(([en, zh, emoji, cat, diff], id) => ({ id, kind: "word", en, zh, emoji, cat, diff }));
const NV = WORDS.length;
// ---------- 會話庫（場景制：一景 1~2 輪你來我往）----------
const THEMES = { greet: "打招呼", polite: "有禮貌", school: "學校", world: "認識世界", food: "吃吃喝喝", play: "一起玩", home: "家", feel: "心情" };
// [theme, icon, diff, [ [prompt, promptZh, reply, replyZh], ... ]]
const SCENE_RAW = [
  ["greet", "🙋", 1, [["Hello!", "哈囉！", "Hi!", "嗨！"]]],
  ["greet", "🌞", 2, [["Good morning!", "早安！", "Good morning!", "早安！"], ["How are you?", "你好嗎？", "I'm fine, thank you!", "我很好，謝謝！"]]],
  ["greet", "🐥", 2, [["What's your name?", "你叫什麼名字？", "My name is Kiki.", "我叫奇奇。"]]],
  ["greet", "👋", 2, [["Bye-bye!", "掰掰！", "See you!", "再見！"], ["Have a nice day!", "祝你有美好的一天！", "You too!", "你也是！"]]],
  ["polite", "🎁", 2, [["Thank you!", "謝謝你！", "You're welcome!", "不客氣！"]]],
  ["polite", "🤝", 2, [["I'm sorry.", "對不起。", "It's OK!", "沒關係！"]]],
  ["polite", "🛎️", 2, [["May I come in?", "我可以進來嗎？", "Sure! Come on in!", "當然，快進來！"]]],
  ["school", "🛟", 2, [["Can you help me?", "你可以幫我嗎？", "Sure! No problem!", "當然，沒問題！"]]],
  ["school", "🔎", 3, [["Where is my pencil?", "我的鉛筆在哪裡？", "It's in your bag!", "在你的書包裡！"], ["Oh, I see. Thank you!", "喔我看到了，謝謝！", "You're welcome, Momo!", "不客氣，蒙蒙！"]]],
  ["school", "🔔", 2, [["Class is over. Line up!", "下課了，排隊！", "OK, Mr. Momo!", "好的，蒙蒙老師！"]]],
  ["food", "🧁", 2, [["Are you hungry?", "你餓了嗎？", "Yes! I want cake!", "餓了！我想吃蛋糕！"], ["Here's your cake.", "你的蛋糕來囉。", "Thank you! It's yummy!", "謝謝！好好吃！"]]],
  ["food", "🍏", 2, [["Do you like apples?", "你喜歡蘋果嗎？", "Yes, I do!", "喜歡！"]]],
  ["food", "🧋", 2, [["What do you want to drink?", "你想喝什麼？", "Milk, please!", "牛奶，麻煩你！"]]],
  ["play", "🛝", 1, [["Let's play!", "一起玩吧！", "OK! Let's go!", "好，走吧！"]]],
  ["play", "🎮", 2, [["Can I play with you?", "我可以跟你一起玩嗎？", "Sure! Let's play together!", "當然！一起玩吧！"], ["It's your turn now.", "換你囉。", "OK! Watch me!", "好！看我的！"]]],
  ["home", "⏰", 2, [["Wake up, Kiki!", "奇奇，起床囉！", "Good morning, Mom!", "媽媽早安！"], ["Time for school!", "該上學囉！", "OK, Mom! Let's go!", "好的媽媽，出發！"]]],
  ["home", "🌛", 2, [["Time for bed.", "該睡覺囉。", "Good night! Sweet dreams!", "晚安，好夢！"]]],
  ["feel", "🤗", 2, [["What's wrong?", "怎麼了？", "I'm sad.", "我難過。"], ["Don't be sad. I'm here!", "別難過，有我在！", "Thank you, Momo!", "謝謝你，蒙蒙！"]]],
  ["greet", "🎂", 2, [["How old are you?", "你幾歲？", "I'm six years old!", "我六歲！"]]],
  ["greet", "🌤️", 1, [["Good afternoon!", "午安！", "Good afternoon, Mr. Momo!", "午安，蒙蒙老師！"]]],
  ["polite", "🙇", 2, [["Excuse me!", "不好意思！", "Yes? Can I help you?", "嗯？需要幫忙嗎？"]]],
  ["school", "🚻", 2, [["Yes, Kiki?", "奇奇，怎麼了？", "May I go to the bathroom, please?", "我可以去上廁所嗎？"]]],
  ["school", "🧍", 2, [["Stand up, please.", "請站起來。", "OK!", "好！"], ["Sit down, please.", "請坐下。", "Yes, Mr. Momo!", "好的，蒙蒙老師！"]]],
  ["school", "🆘", 2, [["What's the matter?", "怎麼了嗎？", "I can't open it. Help me, please!", "我打不開，請幫幫我！"]]],
  ["world", "❓", 2, [["What's this?", "這是什麼？", "It's an apple!", "是蘋果！"]]],
  ["world", "🖌️", 2, [["What color is it?", "這是什麼顏色？", "It's red!", "是紅色！"]]],
  ["world", "🔢", 2, [["How many apples do you see?", "你看到幾顆蘋果？", "I see three apples!", "我看到三顆蘋果！"]]],
  ["world", "⛅", 2, [["How's the weather today?", "今天天氣怎麼樣？", "It's sunny!", "是晴天！"]]],
  ["world", "🏷️", 2, [["Is this your bag?", "這是你的書包嗎？", "Yes, it is!", "對，是我的！"]]],
];
const SCENES = SCENE_RAW.map(([theme, icon, diff, turns], sid) => ({ sid, theme, icon, diff, turns: turns.map(([prompt, promptZh, en, zh]) => ({ prompt, promptZh, en, zh })) }));
const DLGS = [];
SCENES.forEach(sc => sc.turns.forEach((t, ti) => {
  DLGS.push({
    id: NV + DLGS.length, kind: "dlg", en: t.en, zh: t.zh, emoji: sc.icon, cat: "dlg",
    diff: sc.diff, prompt: t.prompt, promptZh: t.promptZh, sid: sc.sid, turn: ti,
  });
}));
const ALL = [...WORDS, ...DLGS];
function sceneLines(item) {              // 整景台詞（教學卡/貼紙簿重播用）
  const sc = SCENES[item.sid];
  const out = [];
  sc.turns.forEach(t => { out.push(t.prompt, t.en); });
  return out;
}
...
const ISLANDS = [
  ...
  { id: "wear", name: "衣物玩具島", icon: "🎽", cats: ["wear"] },
  { id: "out", name: "出門島", icon: "🚏", cats: ["out"] },
...
const MODE_EASE = { teach: 1, listen: 1.1, word2pic: 1.0, letter: 0.95, pic2word: 0.85, spell: 0.8, dlgReply: 0.9, dlgSay: 0.78 };
function stageMode(w, word) {          // 挑戰島專用：只會拿到教過的內容
  if (word.kind === "dlg") return w.s < 0.62 ? "dlgReply" : "dlgSay";  // 聽懂 → 開口說
...
function applyTeach(st, id) {           // 對話以「景」為單位教：兄弟輪一起教
  const it = ALL[id];
  if (it.kind === "dlg") {
    for (const d of DLGS) if (d.sid === it.sid) applyAnswer(st, d.id, true, 0.98, "teach");
  } else applyAnswer(st, id, true, 0.98, "teach");
}
...
function candidatesTeach(st, poolIds) {  // 蒙蒙教室：新內容＋回爐重教
  ...
  const dedupScene = merged.filter(id => {
    const it = ALL[id];
    if (it.kind !== "dlg") return true;
    if (seenSid.has(it.sid)) return false;
    seenSid.add(it.sid); return true;
  });
  const out = dedupScene.slice(0, 5).map(id => ({ id, tag: "new" }));
...
function buildQuestion(st, wordId, tag) {
  ...
  if (mode === "dlgReply" || mode === "dlgSay") {
    const others = shuffled(DLGS.filter(x => x.sid !== word.sid)).slice(0, 2);
...
// —— 2.0 核心：語音 Promise 化。resolve = 這段話「真的說完了」——
//    onend 為主、估時保險絲為輔（行動瀏覽器 onend 偶爾不觸發也不會卡死）
let ttsToken = 0;
let speakingCb = null;
let curCancel = null;                 // 新語音來時，讓上一段立刻讓位
function setSpeakingCb(fn) { speakingCb = fn; }
function notifySpeak(on) { try { if (speakingCb) speakingCb(on); } catch (e) { /* noop */ } }
const estMs = t => 420 + t.length * 95;   // rate 0.8 約 10-11 字/秒的粗估
function speakSeqAsync(parts, rate = 0.8) {
  ...
  return new Promise(resolve => {
    ...
    try {
      if (!hasTTS()) {                     // 沒有語音的裝置：給閱讀節奏，不要閃卡
        notifySpeak(true);
        setTimeout(finish, Math.min(1500, 300 + parts.join(" ").length * 40));
        return;
      }
      ...
      let i = 0, guard = null;
      const next = () => {
        if (my !== ttsToken) { settle(false); return; }   // 被新語音接手：安靜退場
        ...
        guard = setTimeout(step, estMs(text) + 1800);  // 保險絲
...
// 舊介面（fire-and-forget）保留給即點即聽的按鈕
function speak(text, rate = 0.8) { speakAsync(text, rate); return true; }
...
export default function App() {
  ...
  function nextLessonCard(st, ids, num) {
    ...
    setTimeout(() => {
      ...
      setLessonNum(num);
      if (item.kind === "dlg") setTimeout(() => speakSeq(sceneLines(item)), 350);
      else setTimeout(() => speak(item.en), 350);
    }, 30);
  }
  async function tapTeachCard() {
    if (busyTeach.current || !lessonQ) return;
    busyTeach.current = true;
    fx("tap");
    const it = lessonQ.item;
    await speakSeqAsync(it.kind === "dlg" ? sceneLines(it) : [it.en]);
    await sleep(420);                        // 喘口氣再換卡
  ...
  function finishTeach() {
    ...
    setLearner(st); learnerRef.current = st;
    const taughtIds = item.kind === "dlg" ? DLGS.filter(d => d.sid === item.sid).map(d => d.id) : [item.id];
    const nl = [...lessonListRef.current, ...taughtIds.map(id => ({ id, tag: lessonQ.tag }))];
  ...
  function nextQuestion(st, ids, num) {
    ...
    setTimeout(() => {
      ...
      setQNum(num);
      busy.current = false;
      if (qq.mode === "dlgSay") { /* 開口說：不自動放語音 */ }
      else if (qq.mode === "dlgReply") setTimeout(() => speak(qq.word.prompt), 350);
  ...
  function finishAnswer(correct) {
    ...
    if (word.kind === "dlg") {
      const parts = q.mode === "dlgSay" ? [word.prompt, word.en] : [word.en];
      speakSeq(parts, 0.78);
    }
      ...
      <div style={{ maxWidth: 620, margin: "0 auto", padding: "16px 12px 40px", position: "relative" }}>
          ...
          <div>
            ...
            <Card style={{ textAlign: "center" }}>
              ...
              {lessonQ.item.kind !== "dlg" && (
                <button onClick={tapTeachCard} style={{
                  ...
                  <div style={{ fontSize: 15, color: K.sub, fontWeight: 700 }}>{lessonQ.item.zh}　{isSpeaking ? "🎶" : "🔊"}</div>
                </button>
              )}
              {lessonQ.item.kind === "dlg" && (
                <button onClick={tapTeachCard} style={{
                  ...
                  <div style={{ textAlign: "center", fontSize: 40 }}>{lessonQ.item.emoji}
                    {SCENES[lessonQ.item.sid].turns.length > 1 && <span style={{ fontSize: 12, fontWeight: 900, color: K.sunEdge, marginLeft: 6 }}>🔁 {SCENES[lessonQ.item.sid].turns.length} 輪對話</span>}
                  </div>
                  {SCENES[lessonQ.item.sid].turns.map((t, i) => (
                    <div key={i}>
                      <div style={{ display: "flex", gap: 8, alignItems: "flex-start", marginTop: 6 }}>
                        <span style={{ fontSize: 22 }}>🦉</span>
                        <div style={{ background: "#eef6ff", borderRadius: "4px 16px 16px 16px", padding: "7px 11px" }}>
                          <div style={{ fontFamily: MONO, fontWeight: 900, fontSize: 15 }}>{t.prompt}</div>
                          <div style={{ fontSize: 11, color: K.sub }}>{t.promptZh}</div>
                        </div>
                      </div>
                      <div style={{ display: "flex", gap: 8, alignItems: "flex-start", marginTop: 7, flexDirection: "row-reverse" }}>
                        <span style={{ fontSize: 22 }}>🐥</span>
                        <div style={{ background: "#fff4d6", borderRadius: "16px 4px 16px 16px", padding: "7px 11px" }}>
                          <div style={{ fontFamily: MONO, fontWeight: 900, fontSize: 15 }}>{t.en}</div>
                          <div style={{ fontSize: 11, color: K.sub }}>{t.zh}</div>
                        </div>
                      </div>
                    </div>
                  ))}
                  <div style={{ textAlign: "center", fontSize: 12.5, color: K.sub, fontWeight: 700, marginTop: 8 }}>
                    {isSpeaking ? "🎶 蒙蒙說話中…說完會自動下一張" : "🔊 再聽一次＋完成，點這裡"}
                  </div>
                </button>
              )}
              <div style={{ fontSize: 11.5, color: K.sub, marginTop: 10 }}>教室只上課、不考試——挑戰島才有題目喔！{"\u00A0"}語音說完才會換卡 🐢</div>
            </Card>
        ...
        {(phase === "play" || phase === "reveal") && q && (
          <div>
            ...
            <Card style={{ textAlign: "center" }}>
              ...
              {(q.mode === "dlgReply" || q.mode === "dlgSay") && (
                <div>
                  ...
                  {q.word.turn > 0 && (
                    <div style={{ opacity: 0.55, margin: "2px auto 4px", maxWidth: 340 }}>
                      {SCENES[q.word.sid].turns.slice(0, q.word.turn).map((t, i) => (
                        <div key={i} style={{ fontSize: 11, fontFamily: MONO, fontWeight: 700, textAlign: "left", lineHeight: 1.5 }}>
                          🦉 {t.prompt}　<span style={{ color: K.sub }}>🐥 {t.en}</span>
                        </div>
                      ))}
                    </div>
                  )}
                  <div style={{ display: "flex", gap: 8, alignItems: "flex-start", justifyContent: "center", marginTop: 4 }}>
                    ...
                    <button onClick={() => { fx("tap"); speak(q.word.prompt); }} style={{
                      ...
                      <div style={{ fontFamily: MONO, fontWeight: 900, fontSize: 17 }}>{q.word.prompt} <span style={{ fontSize: 13 }}>{q.mode === "dlgSay" ? "🤫" : "🔊"}</span></div>
                      <div style={{ fontSize: 11.5, color: K.sub }}>{q.word.promptZh}</div>
                      {q.mode === "dlgSay" && <div style={{ fontSize: 10.5, color: K.purpleD, fontWeight: 900 }}>這次不放答案語音——想想你會怎麼說！</div>}
                    </button>
                  </div>
                </div>
              )}
            ...
            {q.mode !== "spell" && q.mode !== "teach" && q.mode !== "dlgReply" && q.mode !== "dlgSay" && (
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                {q.options.map((opt, i) => {
            ...
            {phase === "reveal" && (
              <div style={{ textAlign: "center", marginTop: 12 }}>
                ...
                <Btn onClick={goNext} disabled={isSpeaking} color={K.teal} edge={K.tealEdge} fg="#fff" style={{ fontSize: 17, padding: "13px 34px" }}>
                  {isSpeaking ? "🔊 蒙蒙說話中…" : qNum + 1 >= QUIZ_N ? "🎁 看看今天的成果！" : "下一題 ▶"}
                </Btn>
              </div>
            )}
            ...
            <Card>
              ...
              <div style={{ marginBottom: 12 }}>
                {Object.entries(THEMES).map(([tk, tname]) => {
                  const scs = SCENES.filter(sc => sc.theme === tk);
                  ...
                  return (
                    <div key={tk} style={{ marginBottom: 7 }}>
                      ...
                      <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                        {scs.map(sc => {
                          const items = DLGS.filter(d => d.sid === sc.sid);
                          const m = items.every(d => learner.ws[d.id].s >= 0.7);
                          const half = items.some(d => learner.ws[d.id].s >= 0.35);
                          return (
                            <div key={sc.sid} onClick={() => m && speakSeq(sceneLines(items[0]))} style={{
                              ...
                              <span style={{ fontSize: 20, filter: m ? "none" : "grayscale(1)", opacity: m ? 1 : 0.45 }}>{sc.icon}</span>
                              <span style={{ fontSize: 10.5, fontFamily: MONO, fontWeight: 700, color: m ? K.ink : "#b0a488" }}>
                                {m || half ? sc.turns[0].prompt : "?"}
                              </span>
                              {sc.turns.length > 1 && <span style={{ fontSize: 9.5, fontWeight: 900, color: K.sunEdge }}>🔁{sc.turns.length}</span>}
                            </div>
                          );
                        })}
                      </div>
          ...
          {!compact && (
            <>
              ...
              <div style={{ fontSize: 11.5, color: K.sub }}>
                <b>老實說明</b>：這個版本把「教」與「考」拆成兩個空間、兩位 MCTS 規劃師——課程規劃師選「教什麼」時，rollout 會一路模擬到之後的挑戰賽；出題規劃師只考教過的、挑答對率約七成的甜蜜點複習。我們用 30 位「參數刻意不同的合成孩子」重測：<b>拆開這個結構本身就是最大功臣</b>——不論課程用 MCTS、隨機或由易到難安排，真正學會的內容都到 5.7~6.0 個、挫折 0（混合式的冒險島同量互動約 3.3 個）；MCTS 規劃師在此之上再加約 2~5% 綜合分。模擬器仍是遺忘曲線＋測驗效應＋興趣電量的近似模型，每答一題校正一次。字彙 146 個、涵蓋常見一年級主題並補齊 26 個字母的首字（q/v/x/z 由 queen/vest/x-ray/zebra 等補位）；抽象形容詞（big/small/long/short）與功能詞（I/you/this/that）不適合圖卡題型，改由會話句中自然出現，屬已知取捨。3.0 擴充：會話升級為 29 個場景／37 輪（含問年齡、指物、顏色、數數、天氣、上廁所請求、課堂口令、求助、物品歸屬等一年級核心溝通功能）（含多輪對話與「開口說」——只給中文情境、不放語音，從聽懂進到會說）。2.0 修正：換卡與「下一題」都會等語音真的說完（onend 完成偵測＋估時保險絲，行動裝置也不會卡住）。已上課 {seeds} 堂、挑戰 {sessions} 場；進度自動儲存</div>
              {onReset && (
